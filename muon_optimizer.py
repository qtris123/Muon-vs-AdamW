...
class Muon(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        self._rms_interval = max(1, int(rms_interval))
        self._rms_step = 1
        self._last_rms: Optional[Dict[str, float]] = None

    def adjust_lr_for_muon(self, lr, param_shape):
        A, B = param_shape[:2]
        # We adjust the learning rate and weight decay based on the size of the parameter matrix
        # as describted in the paper
        adjusted_ratio = 0.14* math.sqrt(max(A, B))
    ...
    def step(self, closure=None):
        ...
        log_this = (self._rms_step%self._rms_interval) == 0
        muon_sumsq, muon_count = 0.0, 0
        adamw_sumsq, adamw_count = 0.0, 0
        for group in self.param_groups:
            ...
            for p in params:
                ...
                if log_this:
                    A, B = p.shape[:2]
                    uf = (u*math.sqrt(max(A,B))).float() # O_t*sqrt(max(A,B))
                    muon_sumsq += float((uf*uf).sum().item())
                    muon_count += uf.numel()
            ...
            for p in params:
                ...
                if log_this:
                    dt = (g / scale).float() # m^_t / (sqrt(v^_t) + eps) 
                    adamw_sumsq += float((dt*dt).sum().item())
                    adamw_count += g.numel()
        
        if log_this:
            out = {}
            if muon_count > 0:
              out["train/muon_Ot"] = math.sqrt(muon_sumsq / muon_count)
            if adamw_count > 0:
              out["train/adamw_dir"] = math.sqrt(adamw_sumsq / adamw_count)
            if out != {}:
              wandb.log(out)
            print("@@@ RMS Updates: ", out)
